...
// Helper para converter texto em MIME Base64 (padrão Gmail) com suporte a anexos
function createMimeMessage(to, subject, body, threadId = null, replyToId = null, attachmentPath = null) {
    const boundary = "__OtimizaBoundary__";
    ...
    if (attachmentPath && fs.existsSync(attachmentPath)) {
        ...
        // Parte do Anexo
        messageParts.push(`--${boundary}`);
        messageParts.push(`Content-Type: application/pdf; name="${fileName}"`);
...
async function sendEmail(auth, to, nome, icebreaker, empresa, contexto = "") {
    ...
    // Se o lead for um condomínio, usamos uma copy específica de "Vet em Casa"
    if (contexto.toLowerCase().includes("condomínio") || contexto.toLowerCase().includes("administradora")) {
        subject = `benefício Pet-Friendly para o ${empresa}`;
        body = `${saudacao}

${icebreaker}

Escrevo pois estamos lançando uma campanha focada em condomínios residenciais de Belo Horizonte chamada "Vet em Casa".

Nossa proposta é oferecer Vacinação em Domicílio e suporte veterinário básico para os moradores do ${empresa}. Optamos por trabalhar com um **preço social de apenas R$ 20,00 por pet**, garantindo que todos os condôminos tenham acesso a saúde veterinária de qualidade sem sair de casa.

Este serviço não gera qualquer custo ou trabalho administrativo para a administradora, sendo um benefício exclusivo de bem-estar pet que valoriza o condomínio.

**Estou enviando em anexo a nossa proposta comercial detalhada.**

Faz sentido conversarmos 2 minutos sobre como podemos implementar esse benefício para seus moradores?
...
Você já possui alguém cuidando da regularização técnica hoje, ou a porta ainda está aberta para conversarmos?
...
Responsável Técnico`;
    ...
    // Se for condomínio, anexamos a proposta comercial
    if (contexto.toLowerCase().includes("condomínio") || contexto.toLowerCase().includes("administradora")) {
        attachmentPath = path.join(__dirname, '..', '..', 'Proposta Comercial Vet em Casa Otimiza Farmavet.pdf');
    }

    const raw = createMimeMessage(to, subject, body, null, null, attachmentPath);
