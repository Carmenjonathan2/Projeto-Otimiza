...
// Self-test: rodar `node scanner_validade.js --self-test` (não chama Telegram, só testa o modelo)
if (process.argv.includes('--self-test')) {
    (async () => {
        try {
            console.log('[SELF-TEST] scanner_validade.js — testando modelo Gemini (sem imagem real)...');
            const probe = genAI.getGenerativeModel({
                model: "gemini-2.5-flash-lite",
                generationConfig: { maxOutputTokens: 50, temperature: 0 }
            });
            const r = await probe.generateContent('Responda apenas com a palavra: OK');
            console.log(`[SELF-TEST OK] Resposta do modelo: "${r.response.text().trim()}"`);
            process.exit(0);
        } catch (e) {
