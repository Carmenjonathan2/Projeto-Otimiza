...
try:
    ...
    font_nota = ImageFont.truetype("Poppins-Regular.ttf", 28)
    ...
    font_nota = ImageFont.truetype("arial.ttf", 28)
...
products = [
    {
        "filename": "post_librela.png",
        "nome": "Librela (5mg, 10mg, 20mg e 30mg)",
        "icon": "vaccine_vial_icon.png"
    },
    {
        "filename": "post_cytopoint.png",
        "nome": "Cytopoint (10mg, 20mg, 30mg e 40mg)",
        "icon": "vaccine_vial_icon.png"
    },
    {
        "filename": "post_fiv_felv.png",
        "nome": "Teste Rápido FIV/FeLV",
        "icon": "fiv_felv_box.png"
    }
]
...
for p in products:
    ...
    # Load and process product icon
    img_icon = None
    if os.path.exists(p['icon']):
        try:
            img_icon_raw = Image.open(p['icon']).convert("RGBA")
            img_icon = crop_margins(img_icon_raw, tolerance=30)
            ...
            img_icon = img_icon.resize((new_w, new_h), Image.Resampling.LANCZOS)
            img_icon = blend_white_background(img_icon, lilas_off_white)
            
            # Paste in the center of the box
            x_offset = (width - img_icon.width) // 2
            box_center_y = box_y_start + (box_y_end - box_y_start) // 2
            y_offset = box_center_y - (img_icon.height // 2)
            img.paste(img_icon, (x_offset, y_offset))
            print(f"Pasted custom icon {p['icon']} successfully.")
        except Exception as e:
            print(f"Error processing product icon {p['icon']}: {e}")
            
    if img_icon is None:
        draw.text((width//2, 540), "PRODUTO", fill=lilas_medio, font=font_nome, anchor="mm")
    ...
    # Disclaimer: Imagens meramente ilustrativas.
    draw.text((width//2, height - 75), "*Imagens meramente ilustrativas.", fill=lilas_medio, font=font_nota, anchor="mm")
...
print("All new product flyers generated successfully.")
